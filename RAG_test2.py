...
class RAGEvaluationApp:
    ...
    def _evaluate_with_ragas(self, question, context, ground_truth, answer):
        ...
        try:
            # Ensure proper event loop setup for this thread
            self._setup_asyncio()
            ...
            # Compute RAGAS metrics with proper error handling
            try:
                faithfulness_score = faithfulness.score(eval_df).iloc[0]
            except Exception as e:
                st.warning(f"Error calculating faithfulness: {str(e)}")
                faithfulness_score = 0.5
                
            try:
                relevancy_score = answer_relevancy.score(eval_df).iloc[0]
            except Exception as e:
                st.warning(f"Error calculating answer relevancy: {str(e)}")
                relevancy_score = 0.5
                
            try:
                precision_score = context_precision.score(eval_df).iloc[0]
            except Exception as e:
                st.warning(f"Error calculating context precision: {str(e)}")
                precision_score = 0.5
                
            try:
                recall_score = context_recall.score(eval_df).iloc[0]
            except Exception as e:
                st.warning(f"Error calculating context recall: {str(e)}")
                recall_score = 0.5
