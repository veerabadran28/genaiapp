...
# Add timestamp-based versioning function
def get_attestation_version():
    return f"1.0.{int(datetime.datetime.now().timestamp())}"

# Function to hash the attestation content for verification
...
def save_attestation(data):
    ...
    if os.path.exists(filename):
        ...
        # Create columns for user data plus attestation version/content
    ...
    # Add the complete attestation content to the record
    attestation_content = {}
    ...
    # Convert the attestation content to a string representation and hash
    version = get_attestation_version()
    content_hash = hash_attestation_content(attestation_content)
    
    # Create a full text representation of what was attested to
    full_text = ""
...
if not st.session_state.agreed:
    ...
    if st.button("Submit Attestation"):
        if not agree:
            ...
            version, content_hash = save_attestation(st.session_state.user_data)
            
            # Store version and hash in session state
            st.session_state.attestation_version = version
            st.session_state.attestation_hash = content_hash
            
            # Also store which specific points the user attested to
            st.session_state.attested_content = {}
            for section, content in attestation_sections.items():
                if isinstance(content, list):
                    st.session_state.attested_content[section] = content
                else:
                    st.session_state.attested_content[section] = [content]
    ...
    - **Attestation Version**: {st.session_state.attestation_version}
    - **Content Verification Hash**: {st.session_state.attestation_hash[:10]}...
    
    A complete record of all terms you have agreed to has been saved for compliance purposes.
    """)
    
    # Add option to view the complete attestation
    with st.expander("View Complete Attestation Record"):
        st.info("This is the exact content you have attested to:")
        
        for section, items in st.session_state.attested_content.items():
            ...
            if isinstance(items, list):
                for idx, item in enumerate(items, 1):
                    if "**Prohibited Uses**" in item:
                        base_text = item.split(":\n")[0] + ":"
                        sub_items = item.split(":\n")[1].strip().split("\n")
                        st.markdown(f"{idx}. {base_text}")
                        
                        for i, sub_item in enumerate(sub_items, 1):
                            st.markdown(f"    {idx}.{i}. {sub_item.strip('- ')}")
                    elif "**Personal Data Prohibition**" in item:
                        base_text = item.split(":\n")[0] + ":"
                        sub_items = item.split(":\n")[1].strip().split("\n")
                        st.markdown(f"{idx}. {base_text}")
        ...
        st.download_button(
            label="Download Attestation Record",
            data=str(st.session_state.attested_content),
            file_name=f"attestation_{st.session_state.user_data['employee_id']}_{st.session_state.user_data['accepted_date'].replace(' ', '_').replace(':', '-')}.txt",
            mime="text/plain"
        )
