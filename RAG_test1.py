...
class RAGEvaluationApp:
    ...
    def _evaluate_with_ragas(self, question, context, ground_truth, answer):
        ...
        try:
            ...
            scores = {
                "faithfulness": faithfulness.score(eval_df).iloc[0],
                "answer_relevancy": answer_relevancy.score(eval_df).iloc[0],
                "context_precision": context_precision.score(eval_df).iloc[0],
                "context_recall": context_recall.score(eval_df).iloc[0]
            }
            return scores
        except Exception as e:
    ...
    def _fallback_scoring(self, question, context, ground_truth, answer):
        ...
        try:
            ...
            tfidf_matrix = vectorizer.fit_transform([question, answer, ground_truth, context])
            ...
            return {
                "faithfulness": max(faithfulness_score, 0.5),
                "answer_relevancy": max(relevancy_score, 0.5),
                "context_precision": max(context_precision_score, 0.5),
                "context_recall": max(context_recall_score, 0.5)
            }
        except Exception as e:
