...
def load_and_process_sds_data():
    ...
    try:
        ...
        # Clean CpartyId in both DataFrames
        cohouse_df["CpartyId"] = cohouse_df["CpartyId"].astype(str).str.strip().str.replace(r"^0+", "", regex=True).str.lower()
        bcbcusid_df["CpartyId"] = bcbcusid_df["CpartyId"].astype(str).str.strip().str.replace(r"^0+", "", regex=True).str.lower()
        
        # Log sample CpartyId values for debugging
        logger.info(f"Cohouse CpartyId sample: {cohouse_df['CpartyId'].head().tolist()}")
        logger.info(f"BCBCUSID CpartyId sample: {bcbcusid_df['CpartyId'].head().tolist()}")
        
        # Rename MapId columns and clean cohouse
        cohouse_df = cohouse_df.rename(columns={"MapId": "cohouse"})
        cohouse_df["cohouse"] = cohouse_df["cohouse"].astype(str).str.strip().str.replace(r"^0+", "", regex=True)
        ...
        if len(sds_data) == 0:
            logger.warning("No matches found between Cohouse and BCBCUSID sheets on CpartyId. Client data will be empty.")
        
        # Add ClientFlag column as boolean
        sds_data["ClientFlag"] = True
        sds_data["ClientFlag"] = sds_data["ClientFlag"].astype("boolean")  # Use pandas boolean dtype
...
def compute_venn_counts(cd, filtered_data):
    """Compute sets and counts for Venn diagram of in-scope companies, SFF-eligible providers, and clients."""
    try:
        ...
        client_count = len(clients)  # Remove hardcoded value
        client_companies = set(clients["CompanyNumber"].dropna().to_list())
        
        # Compute intersections directly from sets
        ...
        # Compute exclusive counts for venn_counts dictionary
        venn_counts = {
        ...
        return venn_counts, [in_scope_companies, sff_eligible_companies, client_companies]
    except Exception as e:
...
def create_final_dataset(cd, jd, sds_data=None, cre_data=None):
    ...
    if sds_data is not None:
        ...
        final_data["ClientFlag"] = final_data["ClientFlag"].astype("boolean")  # Convert to pandas boolean dtype
        final_data["ClientFlag"] = final_data["ClientFlag"].where(final_data["ClientFlag"].notna(), False)  # Replace NA with False
...
try:
    venn_counts, venn_sets = compute_venn_counts(cd, filtered_data)
    if venn_counts is not None and venn_sets is not None and venn_counts["Total In-scope"] > 0:
        # Display warning if no client data
        if venn_counts["Total Clients"] == 0:
            st.warning("No client data available. The Clients circle will be empty. Check SDS-cohouseid_list.xlsx for data issues.")
        
        # Create Matplotlib Venn diagram
        plt.figure(figsize=(8, 6))
        venn = venn3(
            venn_sets,
            set_labels=('In-scope', 'SFF-eligible', 'Clients'),
            set_colors=('#FF6347', '#228B22', '#4169E1'),  # Red, green, blue
            alpha=0.4
        )
        ...
        for label in venn.subset_labels or []:
            if label:
                ...
                # Format numbers with commas, ensure zeros are displayed
                count = int(label.get_text()) if label.get_text().isdigit() else 0
                label.set_text(f"{count:,}")
        
        plt.title("Venn Diagram: In-scope Companies, SFF-eligible, Clients")
        
        # Save plot to a bytes buffer
        buf = io.BytesIO()
        plt.savefig(buf, format='png', bbox_inches='tight')
        buf.seek(0)
        st.image(buf, caption="Venn Diagram", use_container_width=True)
        plt.close()
        
        # Display note if Clients count is zero
        if venn_counts["Total Clients"] == 0:
            st.write("Note: No clients found in the dataset. Clients circle may appear empty.")
        # Display note if SFF-eligible count is zero
        if venn_counts["Total SFF-eligible"] == 0:
            st.write("Note: No SFF-eligible providers found. SFF-eligible circle may appear empty.")
    else:
