...
class RAGEvaluationApp:
    ...
    def _render_evaluate_rag_page(self):
        ...
        if selected_file:
            try:
                ...
                # Button to trigger RAGAS evaluation
                if st.button("Evaluate with RAGAS", type="primary"):
                    ...
                    # Initialize placeholders for UI feedback
                    ...
                    # Prepare dataset for batch evaluation
                    data = {
                        "question_id": df["Question ID"].tolist(),
                        "question": df["Question"].tolist(),
                        "contexts": [[str(ctx)] for ctx in df["Context"].tolist()],  # Single context per question
                        "ground_truth": df["Ground Truth"].tolist(),
                        "answer": df["Answer Generated"].tolist()
                    }
                    ...
                    try:
                        ...
                        ragas_results = self._evaluate_with_ragas(data)
                        
                        # Process results for each question
                        for idx, row in df.iterrows():
                            question_id = row["Question ID"]
                            result = ragas_results.get(question_id, {
                                "faithfulness": 0.5, "answer_relevancy": 0.5, "context_precision": 0.5,
                                "context_recall": 0.5, "context_entity_recall": 0.5, "answer_similarity": 0.5,
                                "answer_correctness": 0.5
                            })
                            
                            # Append results with all relevant data
                            evaluation_results.append({
                                ...
                                "Overall Score": sum(result.values()) / 7
                            })
                        ...
                        # Log RAGAS failure and attempt LLM fallback for each question
                        st.warning(f"RAGAS evaluation failed: {str(ragas_error)}. Trying LLM evaluation.")
                        
                        # Process each question individually for LLM fallback
                        for idx, row in df.iterrows():
                            try:
                                ...
                                # Log LLM failure and use fallback scoring
                ...
                # Display error if file processing fails
    ...
    def _evaluate_with_ragas(self, dataset):
        """
        Evaluate RAG performance using RAGAS framework for a batch of questions.
        
        Args:
            dataset: A dictionary or HFDataset with 'question', 'contexts', 'ground_truth', and 'answer' lists.
        
        Returns:
            dict: A dictionary mapping question IDs to their RAGAS metric scores.
        """
        try:
            # Check if the dataset is empty or missing required keys
            if not dataset or not all(key in dataset for key in ["question", "contexts", "ground_truth", "answer"]):
                st.warning("Invalid or empty dataset provided for RAGAS evaluation.")
                return {}
            
            # Configure Bedrock client with timeouts and no retries to avoid hanging
            bedrock_config = Config(connect_timeout=120, read_timeout=120, retries={'max_attempts': 0})
            
            # Initialize Bedrock client for AWS API calls
            bedrock_client = boto3.client('bedrock-runtime', config=bedrock_config)
            
            # Set up the LLM using Amazon Nova model for reasoning tasks
            llm = ChatBedrock(model_id="amazon.nova-pro-v1:0", client=bedrock_client)
            
            # Set up embeddings using Titan model for semantic similarity
            embeddings = BedrockEmbeddings(model_id="amazon.titan-embed-text-v2:0", client=bedrock_client)
            
            # Convert input dataset to Hugging Face Dataset if not already
            if not isinstance(dataset, HFDataset):
                hf_dataset = HFDataset.from_dict(dataset)
            else:
                hf_dataset = dataset
            
            # Define the list of RAGAS metrics to evaluate
            metrics = [
                ...
                context_precision,     # Assesses relevance of retrieved context
                context_recall,        # Ensures context contains ground truth info
                context_entity_recall, # Verifies key entities in context
                answer_similarity,     # Compares answer to ground truth semantically
                answer_correctness     # Evaluates factual accuracy and completeness
            ]
            
            # Run RAGAS evaluation on the entire dataset
            ...
            # Initialize dictionary to store results by question ID
            results_dict = {}
            
            # Iterate through results to map scores to question IDs
            for idx, row in result_df.iterrows():
                question_id = dataset.get("question_id", [f"Q{idx+1}"])[idx]  # Use index if no ID provided
                results_dict[question_id] = {
                    "faithfulness": float(row.get("faithfulness", 0.5)),           # Default 0.5 if metric missing
                    "answer_relevancy": float(row.get("answer_relevancy", 0.5)),
                    "context_precision": float(row.get("context_precision", 0.5)),
                    "context_recall": float(row.get("context_recall", 0.5)),
                    "context_entity_recall": float(row.get("context_entity_recall", 0.5)),
                    "answer_similarity": float(row.get("answer_similarity", 0.5)),
                    "answer_correctness": float(row.get("answer_correctness", 0.5))
                }
            
            # Return results mapped to question IDs
            return results_dict
        
        except Exception as e:
            # Log error and re-raise to trigger fallback evaluation
            print(f"RAGAS evaluation failed: {str(e)}")
            raise
    
    def _evaluate_with_llm(self, question, context, ground_truth, answer):
        """
        Evaluate RAG performance using LLM scoring as a fallback.
        
        Args:
            question (str): The input question.
            context (str or list): Retrieved context(s).
            ground_truth (str): Expected answer for comparison.
            answer (str): Generated answer to evaluate.
        
        Returns:
            dict: Scores for each RAGAS metric (0 to 1).
        """
        try:
            # Check for empty or None inputs to avoid processing invalid data
            if not all([answer, question, context, ground_truth]):
                print("Empty inputs detected in LLM evaluation. Returning default scores.")
                return {
            ...
            # Configure Bedrock client with timeouts and no retries
            bedrock_config = Config(connect_timeout=120, read_timeout=120, retries={'max_attempts': 0})
            
            # Initialize Bedrock client for AWS API calls
            bedrock_client = boto3.client('bedrock-runtime', config=bedrock_config)
            
            # Set up LLM using Amazon Nova model for scoring
            llm = ChatBedrock(model_id="amazon.nova-pro-v1:0", client=bedrock_client)
            
            # Convert context to string if it’s a list, for prompt formatting
            context_text = context if isinstance(context, str) else "\n".join(context)
            
            # Truncate context to avoid exceeding token limits (4000 chars)
            max_context_length = 4000
            if len(context_text) > max_context_length:
                context_text = context_text[:max_context_length] + "..."
                print(f"Context truncated to {max_context_length} characters.")
            
            # Define prompts for each metric, asking LLM to score from 0 to 1
            prompts = {
            ...
            # Iterate through each metric and get LLM score
            for metric, prompt in prompts.items():
                try:
                    # Invoke LLM with the metric-specific prompt
                    response = llm.invoke(prompt)
                    
                    # Extract content from response (ChatBedrock returns a message object)
                    ...
                    # Log successful score extraction
                    print(f"LLM score for {metric}: {scores[metric]}")
                    
                except Exception as e:
                    # Log error and use default score if extraction fails
                    print(f"Error evaluating {metric}: {str(e)}")
                    scores[metric] = 0.5
            
            # Log final scores for debugging
            print(f"LLM evaluation scores: {scores}")
            
            # Return scores dictionary
            return scores
        
        except Exception as e:
            # Log error and re-raise to trigger next fallback
            print(f"LLM evaluation failed: {str(e)}")
            raise
    
    def _fallback_scoring(self, question, context, ground_truth, answer):
        """
        Fallback scoring using text similarity metrics (TF-IDF and Jaccard).
        ...
        """
        try:
            # Convert inputs to strings, handling None values
            ...
            # Check if all inputs are empty or whitespace
            ...
            try:
                # Prepare list of texts for TF-IDF
                texts = [question, answer, ground_truth, context]
                
                # Filter out empty texts
                valid_texts = [t for t in texts if t.strip()]
                
                # Check if we have enough texts for similarity computation
                if len(valid_texts) < 2:
                    print("Less than 2 valid texts for TF-IDF. Falling back to Jaccard only.")
                    
                    # Compute Jaccard similarity for all metrics
                    scores = {
                        "faithfulness": jaccard_similarity(answer, context),             # Answer vs Context
                        "answer_relevancy": jaccard_similarity(question, answer),       # Question vs Answer
                        "context_precision": jaccard_similarity(context, ground_truth), # Context vs Ground Truth
                        "context_recall": jaccard_similarity(context, ground_truth),    # Context vs Ground Truth
                        "context_entity_recall": jaccard_similarity(context, ground_truth), # Context vs Ground Truth
                        "answer_similarity": jaccard_similarity(answer, ground_truth),  # Answer vs Ground Truth
                        "answer_correctness": jaccard_similarity(answer, ground_truth)  # Answer vs Ground Truth
                    }
                else:
                    # Compute TF-IDF matrix for valid texts
                    ...
                    # Assign scores based on cosine similarities
                    scores = {
                        ...
                        "context_recall": jaccard_similarity(context, ground_truth),   # Use Jaccard for recall
                        "context_entity_recall": jaccard_similarity(context, ground_truth), # Use Jaccard
                        "answer_similarity": cosine_sim[1, 2],                         # Answer vs Ground Truth
                        "answer_correctness": (cosine_sim[1, 2] + jaccard_similarity(answer, ground_truth)) / 2 # Combine
                    }
            
            except Exception as tfidf_error:
                # Log TF-IDF error and fall back to Jaccard similarity
            ...
            # Log complete failure and return default scores
