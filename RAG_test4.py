...
class BedrockServiceLLM(LLM):
    ...
    def _call(self, prompt: str, stop: Optional[List[str]] = None) -> str:
        """Call the BedrockService with a prompt"""
        # Use the simple invocation method from your service
    ...
    def _evaluate_with_ragas(self, question, context, ground_truth, answer):
        ...
        try:
            ...
            if isinstance(context, str):
                # Split by paragraph or sentence to create a list
                contexts = [c.strip() for c in context.split(". ") if c.strip()]
            else:
                contexts = context
            
            # Create dataset in the format expected by RAGAS 0.2.14
            ...
            if self.bedrock_llm_wrapper:
                # Use the evaluate function from RAGAS 0.2.14 with your Bedrock LLM
                result = evaluate(
                    dataset=dataset,
                ...
                # Fall back to custom scoring if LLM initialization failed
