...
# Add this patching code before you import RAGAS metrics
def _patch_asyncio():
    """Patch the asyncio module to ensure there's always an event loop."""
...
# Apply the patch before importing RAGAS
_patch_asyncio()

# Now import RAGAS metrics
...
class BedrockTitanEmbeddings(Embeddings):
    """Custom embeddings class for AWS Bedrock Titan Embeddings."""
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """Embed a list of documents using Titan Embeddings."""
        embeddings = []
        for text in texts:
    ...
    def embed_query(self, text: str) -> List[float]:
        """Embed a single query string using Titan Embeddings."""
        response = self.client.invoke_model(
            modelId=self.model_id,
            body=json.dumps({"inputText": text}),
            contentType="application/json"
...
# Create a custom LangChain LLM class that wraps your BedrockService
class BedrockServiceLLM(LLM):
    """LangChain wrapper for BedrockService"""
    
    bedrock_service: Any  # Your BedrockService instance
    
    def __init__(self, bedrock_service):
        """Initialize with your existing BedrockService instance"""
        super().__init__()
        self.bedrock_service = bedrock_service
    
    def _call(self, prompt: str, stop: Optional[List[str]] = None, run_manager=None, **kwargs) -> str:
        """Call the BedrockService with a prompt, handling additional arguments.
        
        Args:
            prompt: The input prompt to process
            stop: Optional list of stop strings
            run_manager: Optional run manager (added to handle LangChain's additional argument)
        ...
        print(f"_call received: prompt={prompt}, stop={stop}, run_manager={run_manager}, kwargs={kwargs}")
        
        # Call the underlying service
        response = self.bedrock_service.invoke_model_simple(prompt)
...
# Function to initialize RAGAS with your BedrockService
def initialize_ragas_with_bedrock_service(bedrock_service):
    """Initialize RAGAS metrics with an existing BedrockService instance"""
    try:
        # Create LangChain wrapper around your BedrockService
        bedrock_llm = BedrockServiceLLM(bedrock_service)
        
        # In RAGAS 0.2.14, you don't need to set the LLM directly on metrics
        # Instead, you'll pass it to the evaluate function later
        
        print("Successfully created LangChain wrapper for BedrockService")
        return bedrock_llm
    except Exception as e:
        print(f"Error initializing LangChain wrapper for BedrockService: {str(e)}")
        return None
...
class RAGEvaluationApp:
    def __init__(self, config_path=None):
        ...
        if self.config:
            try:
                ...
                # Initialize RAGAS with your existing Bedrock service
                self.ragas_initialized = initialize_ragas_with_bedrock_service(self.bedrock_service)
    ...
    def _evaluate_with_llm(self, question, context, ground_truth, answer):
        """Evaluate a question-answer pair directly using the Bedrock service."""
        try:
            ...
            # Evaluate faithfulness (answer facts are supported by context)
            ...
            On a scale from 0 to 1, how factually consistent is the answer with the given context?
            0 means the answer contains information not found in the context or contradicts it.
            1 means all information in the answer is supported by the context.
            
            First, analyze what claims the answer makes and whether each is supported by the context.
            Then provide your final score as a single number between 0 and 1.
            """
            ...
            On a scale from 0 to 1, how relevant is this answer to the question?
            0 means the answer is completely unrelated to the question.
            1 means the answer directly and fully addresses what was asked.
            
            First analyze how well the answer addresses the specific question asked.
            Then provide your final score as a single number between 0 and 1.
            """
            
            # Evaluate context precision (context contains relevant information)
            precision_prompt = f"""
            Question: {question}
            ...
            On a scale from 0 to 1, how much of this context is relevant to answering the question?
            0 means none of the context is relevant to the question.
            1 means all parts of the context are relevant to answering the question.
            
            First analyze how much of the context contains information helpful for answering the question.
            Then provide your final score as a single number between 0 and 1.
            """
            
            # Evaluate context recall (context has necessary information)
            recall_prompt = f"""
            Question: {question}
            ...
            On a scale from 0 to 1, how much of the information needed to provide the ground truth answer is present in the context?
            0 means the context is missing all key information needed for the ground truth answer.
            1 means the context contains all the information needed for the ground truth answer.
            
            First analyze what information is needed to provide the ground truth and how much is in the context.
            Then provide your final score as a single number between 0 and 1.
            """
            
            # Get scores from our LLM
            try:
                faithfulness_response = self.bedrock_service.invoke_model_simple(faithfulness_prompt)
                f_score = self._extract_score(faithfulness_response)
            except Exception as e:
                print(f"Faithfulness scoring error: {str(e)}")
                f_score = 0.5
            
            try:
                relevancy_response = self.bedrock_service.invoke_model_simple(relevancy_prompt)
                ar_score = self._extract_score(relevancy_response)
            except Exception as e:
                print(f"Answer relevancy scoring error: {str(e)}")
                ar_score = 0.5
            
            try:
                precision_response = self.bedrock_service.invoke_model_simple(precision_prompt)
                cp_score = self._extract_score(precision_response)
            except Exception as e:
                print(f"Context precision scoring error: {str(e)}")
                cp_score = 0.5
            
            try:
                recall_response = self.bedrock_service.invoke_model_simple(recall_prompt)
                cr_score = self._extract_score(recall_response)
            except Exception as e:
                print(f"Context recall scoring error: {str(e)}")
                cr_score = 0.5
    ...
    def _evaluate_with_ragas(self, question, context, ground_truth, answer):
        """Evaluate a question-answer pair with RAGAS metrics using Bedrock."""
        try:
            if not answer or not question or not context or not ground_truth:
                return {"faithfulness": 0.5, "answer_relevancy": 0.5, "context_precision": 0.5, "context_recall": 0.5}
            ...
            # Initialize Bedrock LLM wrapper (Claude)
            if not hasattr(self, 'bedrock_llm_wrapper'):
                self.bedrock_llm_wrapper = initialize_ragas_with_bedrock_service(self.bedrock_service)
            
            # Initialize Bedrock Titan Embeddings
            if not hasattr(self, 'bedrock_embeddings'):
                self.bedrock_embeddings = BedrockTitanEmbeddings(region_name="us-east-1")
            
            # Perform RAGAS evaluation
            result = evaluate(
                dataset=hf_dataset,
                metrics=[faithfulness, answer_relevancy, context_precision, context_recall],
                llm=self.bedrock_llm_wrapper,
                embeddings=self.bedrock_embeddings
            )
            
            # Debug print to verify result structure
            print(f"RAGAS result: {result}")
            
            # Extract scores from result (take first element of each list)
            return {
    ...
    def _extract_score(self, response):
        ...
        try:
            ...
            # This uses a regex to find floating point or integer numbers
            numbers = re.findall(r"(\d+\.\d+|\d+)", response)
            
            if numbers:
                # Try to get the last number in the response (final score)
                score = float(numbers[-1])
                # Ensure it's between 0 and 1
                return max(0.0, min(score, 1.0))
            
            # If no numbers found, try to see if there's a decimal representation
            if "0." in response:
                parts = response.split("0.")
                if len(parts) > 1:
                    decimal_part = parts[1].strip()
                    # Take just digits until non-digit
                    decimal_digits = ""
            ...
            # For responses like "score: 0.7" or similar
            lower_response = response.lower()
            score_indicators = ["score", "rating", "value", "result"]
            for indicator in score_indicators:
                if indicator in lower_response:
                    parts = lower_response.split(indicator)
                    if len(parts) > 1:
                        after_indicator = parts[1]
                        # Find the first number after the indicator
                        matches = re.findall(r"(\d+\.\d+|\d+)", after_indicator)
                        if matches:
