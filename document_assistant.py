...
class DocumentAssistantUI:
    ...
    def _render_chart_with_values(self, df: pd.DataFrame, chart_type: str, chart_attrs: dict):
        """Render chart with proper data grouping and handling."""
        try:
            # Get column names and ensure proper data types
            if len(df.columns) < 2:
                raise ValueError(f"Insufficient columns in data. Found columns: {df.columns.tolist()}")
                
            category_col = df.columns[0]
            is_three_column = len(df.columns) == 3
            
            # Prepare data frame based on number of columns
            if is_three_column:
                ...
                if chart_type in ['pie', 'donut', 'treemap']:
                    df_grouped = df.groupby([category_col, group_col], as_index=False)[value_col].sum()
                else:
                    df_grouped = df
            else:
            ...
            if is_three_column:
                labels[group_col] = chart_attrs.get('color', group_col)
            ...
            if chart_type == 'pie':
                if is_three_column:
                    ...
                    fig.update_traces(
                        ...
                        textfont_size=12,
                        texttemplate="%{label}<br>Value: %{value:$.2f}M<br>Percent: %{percent:.1%}",
                        hovertemplate="%{label}<br>Value: %{value:$.2f}M<br>Percent: %{percent:.1%}<extra></extra>"
                    )
                ...
                if is_three_column:
                    if chart_type == 'horizontal_bar':
                        ...
                        elif chart_type in ['line', 'scatter', 'bubble']:
            ...
            # Update layout
            range_min = min_val * 0.9 if min_val > 0 else min_val * 1.1
            range_max = max_val * 1.1
            ...
            # Add axes configuration for non-circular charts
            if chart_type not in ['pie', 'donut', 'treemap']:
                if chart_type == 'horizontal_bar':
                    layout_update['xaxis'] = dict(
                        ...
                        range=[np.log10(range_min) if use_log_scale else range_min,
                            np.log10(range_max) if use_log_scale else range_max]
                    )
