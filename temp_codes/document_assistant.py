    ...
    def _render_chart_with_values(self, df: pd.DataFrame, chart_type: str, chart_attrs: dict):
        ...
        try:
            ...
            if len(df.columns) == 2:
                ...
                # For single value grouping, use tuple
                ...
                if chart_type == "pie":
                    ...
                    fig = go.Figure(go.Funnel(
                        y=df_grouped[category_col],
                        x=df_grouped[value_col],
                        textposition="inside",
                    ...
                    fig = px.treemap(
                        df_grouped,
                        path=[(category_col,)],  # Use tuple for path
                        values=value_col,
                        title=chart_attrs.get('title', '')
                    )
                    fig.update_traces(
                ...
                # For grouping operations, use tuples
                ...
                if chart_type == "pie":
                    ...
                    fig = px.sunburst(
                        df_grouped,
                        path=[(category_col,), (group_col,)],  # Use tuples for path
                        values=value_col,
                        title=chart_attrs.get('title', '')
                    )
                    fig.update_traces(
